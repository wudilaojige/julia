...
void jl_extern_c(jl_value_t *declrt, jl_tupletype_t *sigt)
{
    ...
    if (jl_is_abstract_ref_type(declrt)) {
        jl_value_t *declrt0 = jl_tparam0(declrt);
        if (jl_is_typevar(declrt0))
            jl_error("@ccallable: return type Ref should have an element type, not Ref{<:T}");
        if (declrt0 == (jl_value_t*)jl_any_type)
            jl_error("@ccallable: return type Ref{Any} is invalid. Use Any or Ptr{Any} instead.");
        crt = (jl_value_t*)jl_any_type;
    }
    JL_LOCK(&codegen_lock);
    if (!jl_type_mappable_to_c(crt))
